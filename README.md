...
| `regie backup` / `restore` / `doctor` | Home Assistant's own backup through its API; the brain's health, the pins against the tested ones, what drifted | 0.6 |
| `regie pair home.yml --matter --room <area> [--role <role> --at <place>] [--code <code>]` | **the walk's Matter half**: the thing commissioned by the phone (a fresh one — Bluetooth, the phone puts it on the Wi-Fi, Home Assistant's own way) or by a code the server commissions over IP (a thing another controller shares), then adopted into a **proposed row** keyed on its serial — the room is the session, the role and the place are the flags, the name is generated; `apply` rooms and names the device from the row. The Zigbee half — the join window, the interviews, the room's group and bindings — is 0.6 | 0.5 |
| `regie suggest` | the mesh's opinion on rooms, from link quality — suggests, never assigns | 0.6 |
...
| packs | `lighting` — room groups (+ per role, per layout row), motion lights, silent alerts, room cards · the vocabulary: `signals` · `modes` · `scenes` · `fx` (shapes/ the bricks, backends/ the envelopes) · `notify` · `scenarios` | [`src/regie/packs/`](src/regie/packs) |

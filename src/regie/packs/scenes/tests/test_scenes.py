...
def test_the_switch_starts_the_walk_and_a_restart_or_a_reload_resumes_it(rendered):
    ...
    # 0.28.3, watched on the wire: a script re-made by a reload goes off ->
    # unavailable -> off (the registry keeps the entity) - a walk ending on
    # its own, on -> off, never re-arms it (a life with no life today ends at once)
    assert all(t["from"] == "unavailable" and "not_from" not in t for t in back)

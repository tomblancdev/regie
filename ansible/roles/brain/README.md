...
`apply` (the conductor) → `doctor` (the brain's health, read after the
converge: one line per check, red = the brain disagrees with the files —
`regie_doctor_strict` fails the play on it, after the report). Every verb
is the engine's; the role reads `changed` from what the engine prints and
logs no secret — the values go through the environment as
`REGIE_SECRET_<NAME>`.
